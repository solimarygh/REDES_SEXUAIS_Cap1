...
Descrevíamos k como "com quantos machos cada fêmea acasala", com os níveis 5, 10 e 20. O código faz outra coisa.

A fêmea acasala com `min(k, machos que ela aceitou entre os A_max
...
E isso morde bem antes do limite aritmético, porque ela não acasala com os dez que avaliou, e sim
...
São duas coisas. A primeira é que as células k = 10 e k = 20 com A_max = 10
...
Passamos então a gravar a poliandria realizada (grau médio das fêmeas que
...
E reenquadramos k como apetite e não como cota: a fêmea busca até k parceiros; quantos
...
Há uma coisa boa no meio disso. Com A_max = 200 está tudo limpo: todas as curvas atingem o k e o
...
Antes, a seleção natural de viabilidade agia sobre os 200 machos adultos. Quem não
sobrevivia saía, e os que sobravam acasalavam.

O problema só existia com a seleção natural ligada. Com ela desligada, o código fazia
...
Uma correção. Numa versão anterior deste resumo eu escrevia que o
...
Há uma distinção que vale registrar. As fêmeas que somem da rede somem por um resultado
...
Onde isso mordia: no estudo em que sigma_p varia, pouco, porque ali sigma_z fica fixo. Mas no
...
E o pior é que o confundimento ficava alinhado com o contraste que interessa. Como o pool só
...
A viabilidade passou então a agir sobre os JUVENIS, antes do censo de adultos. Cada
...
Antes, se a fêmea avaliava os machos e não aceitava nenhum, ela acabava acasalando com o
último avaliado. Nenhuma fêmea ficava sem acasalar.

O problema vem de somar isso com a fecundidade neutra: quem acasalou deixa 50 filhotes, independentemente de com
...
Agora quem não aceita ninguém fica sem acasalar e deixa zero filhotes. A
fecundidade continua neutra, como tínhamos combinado.

Isso tem uma consequência para as redes: passam a existir fêmeas de grau zero. Elas são excluídas do
...
Ao rodar o espelho, a variância da preferência caía de 1.0 para 0.08 e
...
O piso era imposto por nós, não emergente. Qualquer resultado sobre
...
Adotamos então o modelo infinitesimal: o desvio de segregação passou a ser
...
Aqui entra uma limitação da nossa implementação, que é a terceira pergunta para você. O resultado
...
O que preocupa aqui em particular: os dois desvios vão em sentidos opostos e se
...
Ele precisa ser independente porque a geração 1 dos outros estudos já é um controle, mas
...
A leitura é direta. Quando todas as fêmeas querem a mesma coisa, a geometria da
