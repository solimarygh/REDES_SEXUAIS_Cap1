...
```{r sim-info-header, echo=FALSE, results='asis'}
cat(sprintf(
  '<div style="font-size:0.78em; color:#777; background:#f5f5f5; border-left:3px solid #bbb; padding:6px 12px; margin:4px 0 20px 0; font-family:monospace;">
  <b>Branch:</b> polyandria5_neutral_100gen &nbsp;|&nbsp;
  <b>N=200</b> | Gen=100 | Réplicas=30 &nbsp;|&nbsp;
  <b>Fecundidade:</b> neutra (F_base=50) &nbsp;|&nbsp;
  <b>Poliandria:</b> 1–5 parceiros por fêmea &nbsp;|&nbsp;
  <b>A_max:</b> {200, 40, 10} &nbsp;|&nbsp;
  <b>Gerado em:</b> %s
  </div>',
...
<p style="font-size:0.85em; color:#666; margin:0 0 12px 0;">

<b>Linha 1:</b> Estrutura da rede (comunidades Louvain)  \|  <b>Linha 2:</b> Evolução em 3 Atos (distribuições de z e p)  \|  <b>Esquerda:</b> σp = 0.5  \|  <b>Direita:</b> σp = 2.0  \|  A_max = 200

</p>

### Aleatória

```{r rede-hist-uniform, echo=FALSE, out.width="49%", fig.show="hold", fig.cap=c("Rede — Aleatória | σp=0.5","Rede — Aleatória | σp=2.0","Evolução em 3 Atos — Aleatória | σp=0.5","Evolução em 3 Atos — Aleatória | σp=2.0")}
base <- "Resultados_Artigo/Redes_Representativas/Graficos"
imgs <- file.path(base, c(
  "Rede_uniform_sigmap0.5_Amax200.png",
  "Rede_uniform_sigmap2.0_Amax200.png",
  "Histogramas_uniform_sigmap0.5_Amax200.png",
  "Histogramas_uniform_sigmap2.0_Amax200.png"
))
existing <- imgs[file.exists(imgs)]
if (length(existing) > 0) knitr::include_graphics(existing) else cat("*Rode o Script 06 primeiro.*")
```

### Gaussiana

```{r rede-hist-gaussian, echo=FALSE, out.width="49%", fig.show="hold", fig.cap=c("Rede — Gaussiana | σp=0.5","Rede — Gaussiana | σp=2.0","Evolução em 3 Atos — Gaussiana | σp=0.5","Evolução em 3 Atos — Gaussiana | σp=2.0")}
base <- "Resultados_Artigo/Redes_Representativas/Graficos"
imgs <- file.path(base, c(
  "Rede_gaussian_sigmap0.5_Amax200.png",
  "Rede_gaussian_sigmap2.0_Amax200.png",
  "Histogramas_gaussian_sigmap0.5_Amax200.png",
  "Histogramas_gaussian_sigmap2.0_Amax200.png"
))
existing <- imgs[file.exists(imgs)]
if (length(existing) > 0) knitr::include_graphics(existing) else cat("*Rode o Script 06 primeiro.*")
```
...
```{r rede-hist-sigmoid, echo=FALSE, out.width="49%", fig.show="hold", fig.cap=c("Rede — Sigmoide | σp=0.5","Rede — Sigmoide | σp=2.0","Evolução em 3 Atos — Sigmoide | σp=0.5","Evolução em 3 Atos — Sigmoide | σp=2.0")}
base <- "Resultados_Artigo/Redes_Representativas/Graficos"
imgs <- file.path(base, c(
  "Rede_sigmoid_sigmap0.5_Amax200.png",
  "Rede_sigmoid_sigmap2.0_Amax200.png",
  "Histogramas_sigmoid_sigmap0.5_Amax200.png",
  "Histogramas_sigmoid_sigmap2.0_Amax200.png"
))
existing <- imgs[file.exists(imgs)]
if (length(existing) > 0) knitr::include_graphics(existing) else cat("*Rode o Script 06 primeiro.*")
```
...
```{r rede-hist-ushaped, echo=FALSE, out.width="49%", fig.show="hold", fig.cap=c("Rede — Disruptiva | σp=0.5","Rede — Disruptiva | σp=2.0","Evolução em 3 Atos — Disruptiva | σp=0.5","Evolução em 3 Atos — Disruptiva | σp=2.0")}
base <- "Resultados_Artigo/Redes_Representativas/Graficos"
imgs <- file.path(base, c(
  "Rede_u-shaped_sigmap0.5_Amax200.png",
  "Rede_u-shaped_sigmap2.0_Amax200.png",
  "Histogramas_u-shaped_sigmap0.5_Amax200.png",
  "Histogramas_u-shaped_sigmap2.0_Amax200.png"
))
existing <- imgs[file.exists(imgs)]
if (length(existing) > 0) knitr::include_graphics(existing) else cat("*Rode o Script 06 primeiro.*")
```
...
A função $P_{ij} = 1 - \exp(-s_i(z_j - p_i)^2)$ penaliza machos próximos da preferência da fêmea e favorece os extremos. Em conflito com a seleção viabilidade (estabilizadora em φ = 5), o equilíbrio pode gerar dois atratores fenotípicos. A rede modular reflecte essa bipartição? Machos do morfo "alto" e do morfo "baixo" pertencem a comunidades distintas com conjuntos de fêmeas separados? A Secção 2.7 testa isso formalmente.
...
# Atualização: Sugestões do Miudo — k fixo por fêmea e controle sem seleção natural

Miudo sugeriu duas modificações ao desenho original (no qual cada fêmea sorteava entre 1 e 5 parceiros, sempre sob seleção natural):

1.  **k fixo por fêmea**: cada fêmea acasala com exatamente $k$ machos, $k \in \{5, 10, 20\}$, em vez de sortear $U[1,5]$ parceiros. Isso permite controlar diretamente o grau de poliandria.
2.  **Controle sem seleção natural** ($V_j = 1$ para todos os machos): isola o efeito *puro* da preferência feminina sobre a topologia, sem a viabilidade diferencial $V_j = \exp(-0.2(z_j-\phi)^2)$.

Esta seção testa essas modificações na grade completa: 4 curvas de preferência × $\sigma_p \in \{0.5, 1.0, 2.0\}$ × $k \in \{5, 10, 20\}$, sempre com $A_{max}=200$ e **sem seleção natural**, 30 réplicas (Fase5_MiudoV2). O valor $\sigma_p = 1.0$ corresponde ao *tipping point* identificado na Secção 2.1 ($\sigma_p = \sigma_z$).

## Assinatura Topológica (σp) e Mudança Evolutiva {.tabset}
...
```{r fase4-plotA, echo=FALSE, fig.cap="Modularidade, Aninhamento, Is e Centralização vs. variação da preferência feminina (σp), na geração final, para A_max=200."}
img <- "Resultados_Artigo/Fase5_MiudoV2/Graficos/Fase4_PlotA_AssinaturaTopologica.png"
if (file.exists(img)) knitr::include_graphics(img) else cat("*Rode o Script 07 (Espiador de Backups) primeiro.*")
```

### Mudança Gen 1 → Gen Final

```{r fase4-plotE, echo=FALSE, fig.cap="Mudança na Modularidade e Aninhamento entre a Geração 1 e a Geração final (σp=2.0), para A_max = 200, 40 e 10."}
img <- "Resultados_Artigo/Fase5_MiudoV2/Graficos/Fase4_PlotE_Dumbell_Gen1vsGenFinal.png"
if (file.exists(img)) knitr::include_graphics(img) else cat("*Rode o Script 07 (Espiador de Backups) primeiro.*")
```

## Painéis Comparativos: k × σp {.tabset}

<p style="font-size:0.85em; color:#666; margin:0 0 12px 0;">

Cada painel mostra a rede representativa para $k \in \{5,10,20\}$ (colunas) e $\sigma_p \in \{2.0, 1.0, 0.5\}$ (linhas, de cima para baixo). A_max=200, sem seleção natural.

</p>

### Aleatória

```{r painel-miudov2-uniform, echo=FALSE, fig.cap="**Figura 11.** Aleatória — comparação de k e σp (A_max=200, sem seleção natural)."}
img <- "Resultados_Artigo/Redes_Representativas/Graficos/Comparacao_k_sigmap_uniform_Amax200_noNS.png"
if (file.exists(img)) knitr::include_graphics(img) else cat("*Rode `gerar_lote_comparativo(200)` no Script 06 primeiro.*")
```

### Gaussiana

```{r painel-miudov2-gaussian, echo=FALSE, fig.cap="**Figura 12.** Gaussiana — comparação de k e σp (A_max=200, sem seleção natural)."}
img <- "Resultados_Artigo/Redes_Representativas/Graficos/Comparacao_k_sigmap_gaussian_Amax200_noNS.png"
if (file.exists(img)) knitr::include_graphics(img) else cat("*Rode `gerar_lote_comparativo(200)` no Script 06 primeiro.*")
```

### Sigmoide

```{r painel-miudov2-sigmoid, echo=FALSE, fig.cap="**Figura 13.** Sigmoide — comparação de k e σp (A_max=200, sem seleção natural)."}
img <- "Resultados_Artigo/Redes_Representativas/Graficos/Comparacao_k_sigmap_sigmoid_Amax200_noNS.png"
if (file.exists(img)) knitr::include_graphics(img) else cat("*Rode `gerar_lote_comparativo(200)` no Script 06 primeiro.*")
```

### Disruptiva

```{r painel-miudov2-ushaped, echo=FALSE, fig.cap="**Figura 14.** Disruptiva — comparação de k e σp (A_max=200, sem seleção natural)."}
img <- "Resultados_Artigo/Redes_Representativas/Graficos/Comparacao_k_sigmap_u-shaped_Amax200_noNS.png"
if (file.exists(img)) knitr::include_graphics(img) else cat("*Rode `gerar_lote_comparativo(200)` no Script 06 primeiro.*")
```

### Interpretação

-   **k controla diretamente o aninhamento e a fragmentação**: em todas as curvas, aumentar $k$ (5 → 10 → 20) reduz a modularidade e aumenta o NODF — redes com mais cópulas por fêmea ficam menos compartimentadas e mais hierárquicas/aninhadas, com menos tribos.
-   **A curva Aleatória é insensível a σp** (Mod e NODF praticamente idênticos entre σp=0.5 e σp=2.0 para o mesmo k), como esperado de uma referência nula — bom *sanity check* dos dados.
-   **A curva Gaussiana mostra forte interação k × σp**: com $k=5$, $\sigma_p=2.0$ fragmenta a rede em até 14 tribos, enquanto $\sigma_p=0.5$ mantém uma única componente conexa. Com $k=20$ essa diferença praticamente desaparece — **k alto "dilui" o efeito de σp** sobre a fragmentação.
-   O valor $\sigma_p=1.0$ (tipping point, linha do meio nos painéis) tende a posicionar-se entre os extremos $\sigma_p=0.5$ e $\sigma_p=2.0$, consistente com a ideia de que é o ponto onde a variação da preferência feminina iguala a variação inicial do traço masculino ($\sigma_z=1.0$).

> **Nota:** estes painéis mostram $A_{max}=200$ (amostragem irrestrita). Os mesmos cenários para $A_{max}=40$ e $A_{max}=10$ (quando as fêmeas "veem" menos machos) já foram gerados e estão disponíveis no HTML de exploração (`Exploracao_Paineis_MiudoV2.Rmd`), para investigar como a restrição de amostragem afeta essas interações k × σp.
...
<script src="https://utteranc.es/client.js"
        repo="solimarygh/REDES_SEXUAIS_Cap1"
        issue-term="title"
        theme="github-light"
