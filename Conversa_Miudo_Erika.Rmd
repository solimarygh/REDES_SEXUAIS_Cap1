...
```{r tabela-miudov2, echo=FALSE}
csv_miudo <- "Resultados_Artigo/Redes_Representativas/Dados/Tabela_Comparativa_MiudoV2_Amax200.csv"
if (file.exists(csv_miudo)) {
  read.csv(csv_miudo) %>%
    kable(caption = "**Tabela 3.** Modularidade, NODF (aninhamento), número de tribos (componentes conexos do Laplaciano) e comunidades (Louvain) para a rede representativa de cada combinação Curva × σp × k (A_max=200, sem seleção natural, 30 réplicas).",
          align = c("l","c","c","r","r","r","r"),
          digits = 3) %>%
    kable_styling(bootstrap_options = c("striped","hover","condensed"),
                  full_width = FALSE, font_size = 13)
} else {
  cat("*Tabela não encontrada — rode `gerar_lote_comparativo(200)` no Script 06 primeiro.*")
}
```
...
> **Nota:** estes resultados focam em $A_{max}=200$ (amostragem irrestrita). Os mesmos cenários para $A_{max}=40$ e $A_{max}=10$ (quando as fêmeas "veem" menos machos) podem ser gerados com `gerar_lote_comparativo(40)` / `gerar_lote_comparativo(10)` no Script 06, para investigar se a restrição de amostragem atenua essas interações k × σp.
