...
**Variância do traço (Var z):** os valores na geração 100 são substancialmente reduzidos em relação ao valor inicial ($\sigma_z^2 = 1.0$), com reduções de 88-90% em todos os regimes de preferência. A diferença entre $A_{max}$ é, na maioria dos casos, pequena. A excepção mais visível é a **curva gaussiana**: sob $A_{max}=10$, Var z é consistentemente mais elevada do que sob $A_{max}=200$, sugerindo que a restrição de amostragem reduz a eficácia do acasalamento assortativo e preserva mais variância. Será que esse efeito de $A_{max}$ sobre a diversidade genética é específico da gaussiana, ou aparece também nas outras curvas a $\sigma_p$ mais altos? (Ver Tabela 1 para os valores exactos.)
...
- **Modularidade → Var(z):** correlação positiva. Redes mais modulares estão associadas a maior variância genética. A u-shaped (maior modularidade) apresenta Var z mais elevado; a sigmoide (menor modularidade) apresenta Var z mais baixo. Será que é a estrutura da rede que causa a diferença, ou ambas são consequências independentes da curva de preferência? (Ver Secção 2.5.)
- **Aninhamento → z̄:** correlação positiva para a sigmoide. Réplicas com maior aninhamento apresentam maior deslocamento de z̄. O mecanismo pelo qual o aninhamento se traduz em exageração do traço é explorado nas Secções 2.5 e 3.
